...
(function () {
  ...
  init = async function () {
    ...
    if (settings.modules.neos.enabled) {
      ...
      // YGO Legacy: Initialize WebSocket adapter
      wsAdapter = createWebSocketHandler(settings.modules.neos.websocket);
      neosWsServer.on('connection', neosRequestListener);
      neosHttpServer.listen(settings.modules.neos.port);
      log.info('Neos WebSocket server started', settings.modules.neos.port);
    }
  ...
  // YGO Legacy: WebSocket adapter factory
  ...
  neosRequestListener = function (client, req) {
    ...
    // YGO Legacy: Apply WebSocket adapter (keepalive, etc.)
    if (wsAdapter) {
      wsAdapter.setup(client);
    }
